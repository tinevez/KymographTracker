...
public class InteractiveMultipleDjikstraTracingESC extends Overlay implements InteractiveMultipleTracing
{
	...
	public InteractiveMultipleDjikstraTracingESC(final Sequence seq, final double alpha, final boolean verticalPath) throws IllegalArgumentException
	{
		...
		if (seq != null)
		{
			...
			if (verticalPath)
				announceFrame = new AnnounceFrame("Tracing paths "+seq.getName()+" from top to bottom: click in image to add intermediary points, press ESC to finalize path ", "Stop", stopTracerThr, 0);
			else
				announceFrame = new AnnounceFrame("Tracing paths "+seq.getName()+": click in image to add intermediary points, press ESC to finalize path ", "Stop", stopTracerThr, 0);
			if (!seq.getViewers().isEmpty())
				seq.getViewers().get(0).toFront();
		}
		else
	...
	public void keyReleased(final KeyEvent e, final Point2D imagePoint, final IcyCanvas canvas)
	{
		if (e.getKeyCode() == KeyEvent.VK_ESCAPE)
		{
			if (this.state == DrawingState.SECOND_CLICKED)
			{
				...
				finalizePath(xInit, yInit, false);
				paused = true;
			}
	...
	class ComputeMapThreadBidirectional extends ComputeMapThread
	{
		...
		{	
			...
			mapThreadListLock.unlock();
			seq.overlayChanged(InteractiveMultipleDjikstraTracingESC.this);
		}
	}
	...
	class ComputePathThread extends StoppableThread implements ActionListener
	{
		...
		private void runFunction()
		{
			...
			if (run)
			{
				...
				pathLock.unlock();
				seq.overlayChanged(new OverlayEvent(null, OverlayEventType.PAINTER_CHANGED));
			}
		}
